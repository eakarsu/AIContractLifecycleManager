/**
 * Custom Feature: portfolio-analytics
 ...
 * Integration credentials: process.env.FEATURE_PORTFOLIO_ANALYTICS_KEY
...
const FEATURE_KEY = process.env.FEATURE_PORTFOLIO_ANALYTICS_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Portfolio analytics & benchmarking.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIContractLifecycleManager - portfolio-analytics',
    },
    body: JSON.stringify({
...
router.post('/portfolio-analytics', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_PORTFOLIO_ANALYTICS_KEY');
    const ai = await callLLM({ feature: 'portfolio-analytics', goal: 'Portfolio analytics & benchmarking', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['portfolio-analytics', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[portfolio-analytics] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'portfolio-analytics',
      endpoint: '/api/ai/portfolio-analytics',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
...
router.get('/portfolio-analytics/health', (req, res) => {
  res.json({
    feature: 'portfolio-analytics',
    endpoint: '/api/ai/portfolio-analytics',
