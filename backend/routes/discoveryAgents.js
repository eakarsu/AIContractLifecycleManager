...
router.post('/analyze-relevance', auth, aiRateLimiter, async (req, res) => {
  try {
...
router.post('/check-privilege', auth, aiRateLimiter, async (req, res) => {
  try {
...
router.post('/extract-timeline', auth, aiRateLimiter, async (req, res) => {
  try {
    const { document_text } = req.body;
